...
public class PortalSessionRepository extends AbstractSessionDAO {

    private static final Logger log = LoggerFactory.getLogger(PortalSessionRepository.class);
    ...
    public void update(Session session) throws UnknownSessionException {
        try {
            sessionRedisTemplate.opsForValue().set(getKey(session.getId().toString()), session,PortalSessionConstants.DEFAULT_SESSION_TIME_OUT, TimeUnit.MINUTES);
        } catch (Exception e) {
    ...
    public Collection<Session> getActiveSessions() {
        List<Session> values = (List<Session>) sessionRedisTemplate.opsForValue().get(
            PortalSessionConstants.XPORTAL_SHIRO_USER_SESSION_CACHE_KEY + "*");
